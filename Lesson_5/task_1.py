...
while(True):
    inp = input("Please, enter name and income (for each quarter) of a company via space to add it or press '-1' to exit.\nFor example: 'Apple 100 200 300 400'\n").split(" ")
    if (inp [0] == "-1"):
        break
    companies.append(company(inp[0],[int(i) for i in inp[1:]]))

average_inc = sum([sum(i.income) for i in companies]) / len(companies)
above_avg = [sum(i.income) > average_inc for i in companies]
